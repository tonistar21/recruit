...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('stepRegisterBtn').addEventListener('click', async () => {
    if (!verifyActionPermission('Створення анкет')) return;

    // Реєструємо через DB
    const newCandidate = await DB.addCandidate(stepperCandidateData);
  ...
  function renderDocumentsTable() {
    ...
    tbody.querySelectorAll('.btn-doc-verify').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        await DB.updateDocumentStatus(cid, did, 'Підтверджено');
    ...
    tbody.querySelectorAll('.btn-doc-reject').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        const cid = e.currentTarget.getAttribute('data-cid');
        const did = e.currentTarget.getAttribute('data-did');
        await DB.updateDocumentStatus(cid, did, 'Відхилено');
    ...
    tbody.querySelectorAll('.btn-doc-delete').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (confirm('Ви дійсно бажаєте безповоротно вилучити цей файл кандидата?')) {
          await DB.deleteDocument(cid, did);
  ...
  async function moveCandidateStageShift(candId, direction) {
    ...
    if (targetIdx >= 0 && targetIdx < STAGES.length) {
      ...
      await DB.updateCandidateStage(candId, targetStage);
  ...
  document.getElementById('editUserSubmitBtn').addEventListener('click', async () => {
    ...
    await DB.updateUser(selectedUserId, payload);
  ...
  function renderRolesMatrix() {
    ...
    tbody.querySelectorAll('.perm-checkbox-cell').forEach(cell => {
      cell.addEventListener('click', async (e) => {
        ...
        await DB.updatePermission(permName, roleName, newVal);
  ...
  document.getElementById('editCandidateSubmitBtn').addEventListener('click', async () => {
    ...
    await DB.updateCandidate(selectedCandidateId, payload);
  ...
  document.getElementById('confirmDeleteBtn').addEventListener('click', async () => {
    await DB.deleteCandidate(selectedCandidateId);
